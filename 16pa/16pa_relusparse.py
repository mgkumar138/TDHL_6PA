...
from backend_scripts.utils import find_cue, saveload, get_default_hp, savefigformats, save_rdyn
import time as dt
...
def run_multiple_expt(b, mtype, env, hp, agent, alldyn, sessions, useweight=None, nocue=None, noreward=None):
    ...
    for t in range(sessions * len(env.rlocs)):
        ...
        if t in env.nort:
            ...
            mvpath[env.idx] = np.array(env.tracks)[:env.normax]
            dgr.append(env.dgr)
        else:
...
def multiplepa_script(hp, b):
    ...
    # Start Training
    lat, dgr, pi, mvpath = run_multiple_expt(b, 'train', env, hp, agent, alldyn, trsess, noreward=nonrp)

    print('---------------- Agent {} done in {:3.2f} min ---------------'.format(b, (dt.time() - start) / 60))

    if hp['savemodel']:
        saveload('save', 'model_th{}_hiddenrelu_{}'.format(hp['sparsity'], dt.time()),
...
def main_script(hp):
    print('Number of PA: {}, sparse sparsity: {}'.format(hp['npa'], hp['sparsity']))
    btstp = hp['btstp']
    ...
    for b in range(btstp):
        totlat[b], totdgr[b], totpi[b], totpath, alldyn = x[b]

    return np.mean(totlat, axis=0), np.mean(totdgr, axis=0), np.mean(totpi, axis=0), \
           np.std(totlat, axis=0), np.std(totdgr, axis=0), np.std(totpi, axis=0), totpath, alldyn
...
if __name__ == '__main__':
    ...
    hp['exptname'] = 'path_dyn_3sparse_16pa_{}_{}_{}ha_{}lr_{}wkm_{}dt_b{}_{}'.format(
    ...
    for n, param in enumerate(totparams):
        ...
        totlat[n], totdgr[n], totpi[n], stdlat[n], stddgr[n], stdpi[n], mvpath, alldyn = main_script(hp)

        saveload('save', '{}'.format(hp['exptname']), [totlat, totdgr, totpi, stdlat, stddgr, stdpi, mvpath, alldyn])
