...
from backend_scripts.utils import find_cue, get_default_hp, savefigformats
import time as dt
...
def main_script(hp):
    print('Number of PA: {}, Taug: {}'.format(hp['npa'], hp['taug']))
    btstp = hp['btstp']
...
if __name__ == '__main__':
    ...
    hp['hidact'] = 'relu'
    totparams = np.logspace(2,4,11)  # TD gamma
    ...
    ax11.set_xlabel('TauG')
    ax11.set_title('Number of PAs learnt with TauG')
    ax11.set_ylabel('PAs with visit ratio > {}%'.format(pithres))

    f1.tight_layout()
    plt.show()

    savefigformats('16pa_taug')
